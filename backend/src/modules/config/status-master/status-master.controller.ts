...
import { CreateStatusMasterSchema, CreateStatusMasterDto } from './dto/create-status-master.dto';
import { UpdateStatusMasterSchema, UpdateStatusMasterDto } from './dto/update-status-master.dto';
import { FilterStatusMasterSchema, FilterStatusMasterDto } from './dto/filter-status-master.dto';
...
@ApiTags('config/status-master')
@ApiBearerAuth()
@Controller('config/status-master')
export class StatusMasterController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateStatusMasterSchema)) dto: CreateStatusMasterDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterStatusMasterSchema)) query: FilterStatusMasterDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateStatusMasterSchema)) dto: UpdateStatusMasterDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'colorCode', label: 'Color Code', type: 'string' },
    { key: 'textColor', label: 'Text Color', type: 'string' },
    { key: 'sequenceNo', label: 'Sequence No', type: 'number' },
    { key: 'isFinalStatus', label: 'Is Final Status', type: 'boolean' },
    { key: 'isDefault', label: 'Is Default', type: 'boolean' },
    { key: 'description', label: 'Description', type: 'string' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'status_master';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="status_master_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="status_master_${Date.now()}.xlsx"`);
    res.send(buf);
  }
