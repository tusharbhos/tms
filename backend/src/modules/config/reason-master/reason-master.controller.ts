...
import { CreateReasonMasterSchema, CreateReasonMasterDto } from './dto/create-reason-master.dto';
import { UpdateReasonMasterSchema, UpdateReasonMasterDto } from './dto/update-reason-master.dto';
import { FilterReasonMasterSchema, FilterReasonMasterDto } from './dto/filter-reason-master.dto';
...
@ApiTags('config/reason-master')
@ApiBearerAuth()
@Controller('config/reason-master')
export class ReasonMasterController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateReasonMasterSchema)) dto: CreateReasonMasterDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterReasonMasterSchema)) query: FilterReasonMasterDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateReasonMasterSchema)) dto: UpdateReasonMasterDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'reasonType', label: 'Reason Type', type: 'string' },
    { key: 'reasonCode', label: 'Reason Code', type: 'string' },
    { key: 'reasonText', label: 'Reason Text', type: 'string' },
    { key: 'requiresNote', label: 'Requires Note', type: 'boolean' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'reason_master';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="reason_master_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="reason_master_${Date.now()}.xlsx"`);
    res.send(buf);
  }
