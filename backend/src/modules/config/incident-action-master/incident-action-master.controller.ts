...
import { CreateIncidentActionMasterSchema, CreateIncidentActionMasterDto } from './dto/create-incident-action-master.dto';
import { UpdateIncidentActionMasterSchema, UpdateIncidentActionMasterDto } from './dto/update-incident-action-master.dto';
import { FilterIncidentActionMasterSchema, FilterIncidentActionMasterDto } from './dto/filter-incident-action-master.dto';
...
@ApiTags('config/incident-action-master')
@ApiBearerAuth()
@Controller('config/incident-action-master')
export class IncidentActionMasterController {
  constructor(private readonly service: IncidentActionMasterService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateIncidentActionMasterSchema)) dto: CreateIncidentActionMasterDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterIncidentActionMasterSchema)) query: FilterIncidentActionMasterDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateIncidentActionMasterSchema)) dto: UpdateIncidentActionMasterDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'actionCode', label: 'Action Code', type: 'string' },
    { key: 'actionName', label: 'Action Name', type: 'string' },
    { key: 'description', label: 'Description', type: 'string' },
    { key: 'defaultSlaHours', label: 'Default Sla Hours', type: 'number' },
    { key: 'isMandatory', label: 'Is Mandatory', type: 'boolean' },
    { key: 'sequenceOrder', label: 'Sequence Order', type: 'number' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'incident_action_master';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="incident_action_master_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="incident_action_master_${Date.now()}.xlsx"`);
    res.send(buf);
  }
