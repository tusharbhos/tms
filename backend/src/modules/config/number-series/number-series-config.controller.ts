...
import { CreateNumberSeriesConfigSchema, CreateNumberSeriesConfigDto } from './dto/create-number-series-config.dto';
import { UpdateNumberSeriesConfigSchema, UpdateNumberSeriesConfigDto } from './dto/update-number-series-config.dto';
import { FilterNumberSeriesConfigSchema, FilterNumberSeriesConfigDto } from './dto/filter-number-series-config.dto';
...
@ApiTags('config/number-series')
@ApiBearerAuth()
@Controller('config/number-series')
export class NumberSeriesConfigController {
  constructor(private readonly service: NumberSeriesConfigService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateNumberSeriesConfigSchema)) dto: CreateNumberSeriesConfigDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterNumberSeriesConfigSchema)) query: FilterNumberSeriesConfigDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateNumberSeriesConfigSchema)) dto: UpdateNumberSeriesConfigDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'currentRunningNo', label: 'Current Running No', type: 'number' },
    { key: 'minDigits', label: 'Min Digits', type: 'number' },
    { key: 'suffix', label: 'Suffix', type: 'string' },
    { key: 'separator', label: 'Separator', type: 'string' },
    { key: 'resetType', label: 'Reset Type', type: 'string' },
    { key: 'lastResetAt', label: 'Last Reset At', type: 'date' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' },
    { key: 'exampleOutput', label: 'Example Output', type: 'string' }
  ];
  private readonly excelSheetName = 'number_series_config';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="number_series_config_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="number_series_config_${Date.now()}.xlsx"`);
    res.send(buf);
  }
