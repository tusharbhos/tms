...
import { CreatePrintTemplatesSchema, CreatePrintTemplatesDto } from './dto/create-print-templates.dto';
import { UpdatePrintTemplatesSchema, UpdatePrintTemplatesDto } from './dto/update-print-templates.dto';
import { FilterPrintTemplatesSchema, FilterPrintTemplatesDto } from './dto/filter-print-templates.dto';
...
@ApiTags('config/print-templates')
@ApiBearerAuth()
@Controller('config/print-templates')
export class PrintTemplatesController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreatePrintTemplatesSchema)) dto: CreatePrintTemplatesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterPrintTemplatesSchema)) query: FilterPrintTemplatesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdatePrintTemplatesSchema)) dto: UpdatePrintTemplatesDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'templateType', label: 'Template Type', type: 'number' },
    { key: 'templateName', label: 'Template Name', type: 'string' },
    { key: 'templateCode', label: 'Template Code', type: 'string' },
    { key: 'paperSize', label: 'Paper Size', type: 'string' },
    { key: 'orientation', label: 'Orientation', type: 'string' },
    { key: 'copies', label: 'Copies', type: 'number' },
    { key: 'templateHtml', label: 'Template Html', type: 'string' },
    { key: 'variablesSchema', label: 'Variables Schema', type: 'string' },
    { key: 'includeLogo', label: 'Include Logo', type: 'boolean' },
    { key: 'includeStamp', label: 'Include Stamp', type: 'boolean' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' },
    { key: 'versionNo', label: 'Version No', type: 'number' }
  ];
  private readonly excelSheetName = 'print_templates';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="print_templates_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="print_templates_${Date.now()}.xlsx"`);
    res.send(buf);
  }
