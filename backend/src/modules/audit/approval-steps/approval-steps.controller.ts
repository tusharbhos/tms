...
import { CreateApprovalStepsSchema, CreateApprovalStepsDto } from './dto/create-approval-steps.dto';
import { UpdateApprovalStepsSchema, UpdateApprovalStepsDto } from './dto/update-approval-steps.dto';
import { FilterApprovalStepsSchema, FilterApprovalStepsDto } from './dto/filter-approval-steps.dto';
...
@ApiTags('audit/approval-steps')
@ApiBearerAuth()
@Controller('audit/approval-steps')
export class ApprovalStepsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateApprovalStepsSchema)) dto: CreateApprovalStepsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterApprovalStepsSchema)) query: FilterApprovalStepsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateApprovalStepsSchema)) dto: UpdateApprovalStepsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'approverType', label: 'Approver Type', type: 'string' },
    { key: 'approverUserId', label: 'Approver User Id', type: 'number' },
    { key: 'approverRoleId', label: 'Approver Role Id', type: 'number' },
    { key: 'assignedAt', label: 'Assigned At', type: 'date' },
    { key: 'dueAt', label: 'Due At', type: 'date' },
    { key: 'actedBy', label: 'Acted By', type: 'number' },
    { key: 'action', label: 'Action', type: 'string' },
    { key: 'actedAt', label: 'Acted At', type: 'date' },
    { key: 'remarks', label: 'Remarks', type: 'string' },
    { key: 'status', label: 'Status', type: 'string' }
  ];
  private readonly excelSheetName = 'approval_steps';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="approval_steps_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="approval_steps_${Date.now()}.xlsx"`);
    res.send(buf);
  }
