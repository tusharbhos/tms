...
import { CreateDashboardsSchema, CreateDashboardsDto } from './dto/create-dashboards.dto';
import { UpdateDashboardsSchema, UpdateDashboardsDto } from './dto/update-dashboards.dto';
import { FilterDashboardsSchema, FilterDashboardsDto } from './dto/filter-dashboards.dto';
...
@ApiTags('audit/dashboards')
@ApiBearerAuth()
@Controller('audit/dashboards')
export class DashboardsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateDashboardsSchema)) dto: CreateDashboardsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterDashboardsSchema)) query: FilterDashboardsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateDashboardsSchema)) dto: UpdateDashboardsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'layoutConfig', label: 'Layout Config', type: 'string' },
    { key: 'status', label: 'Status', type: 'string' }
  ];
  private readonly excelSheetName = 'dashboards';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="dashboards_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="dashboards_${Date.now()}.xlsx"`);
    res.send(buf);
  }
