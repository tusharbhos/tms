...
import { CreateScheduledJobsSchema, CreateScheduledJobsDto } from './dto/create-scheduled-jobs.dto';
import { UpdateScheduledJobsSchema, UpdateScheduledJobsDto } from './dto/update-scheduled-jobs.dto';
import { FilterScheduledJobsSchema, FilterScheduledJobsDto } from './dto/filter-scheduled-jobs.dto';
...
@ApiTags('audit/scheduled-jobs')
@ApiBearerAuth()
@Controller('audit/scheduled-jobs')
export class ScheduledJobsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateScheduledJobsSchema)) dto: CreateScheduledJobsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterScheduledJobsSchema)) query: FilterScheduledJobsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateScheduledJobsSchema)) dto: UpdateScheduledJobsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'jobHandler', label: 'Job Handler', type: 'string' },
    { key: 'retryAttempts', label: 'Retry Attempts', type: 'number' },
    { key: 'lastRunAt', label: 'Last Run At', type: 'date' },
    { key: 'nextRunAt', label: 'Next Run At', type: 'date' },
    { key: 'lastRunStatus', label: 'Last Run Status', type: 'string' },
    { key: 'active', label: 'Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'scheduled_jobs';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="scheduled_jobs_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="scheduled_jobs_${Date.now()}.xlsx"`);
    res.send(buf);
  }
