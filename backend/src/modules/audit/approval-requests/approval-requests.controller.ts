...
import { CreateApprovalRequestsSchema, CreateApprovalRequestsDto } from './dto/create-approval-requests.dto';
import { UpdateApprovalRequestsSchema, UpdateApprovalRequestsDto } from './dto/update-approval-requests.dto';
import { FilterApprovalRequestsSchema, FilterApprovalRequestsDto } from './dto/filter-approval-requests.dto';
...
@ApiTags('audit/approval-requests')
@ApiBearerAuth()
@Controller('audit/approval-requests')
export class ApprovalRequestsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateApprovalRequestsSchema)) dto: CreateApprovalRequestsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterApprovalRequestsSchema)) query: FilterApprovalRequestsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateApprovalRequestsSchema)) dto: UpdateApprovalRequestsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'referenceCode', label: 'Reference Code', type: 'string' },
    { key: 'requestedBy', label: 'Requested By', type: 'number' },
  ...
  private readonly excelSheetName = 'approval_requests';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="approval_requests_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="approval_requests_${Date.now()}.xlsx"`);
    res.send(buf);
  }
