...
import { CreateNotificationTemplatesSchema, CreateNotificationTemplatesDto } from './dto/create-notification-templates.dto';
import { UpdateNotificationTemplatesSchema, UpdateNotificationTemplatesDto } from './dto/update-notification-templates.dto';
import { FilterNotificationTemplatesSchema, FilterNotificationTemplatesDto } from './dto/filter-notification-templates.dto';
...
export class NotificationTemplatesController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateNotificationTemplatesSchema)) dto: CreateNotificationTemplatesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterNotificationTemplatesSchema)) query: FilterNotificationTemplatesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateNotificationTemplatesSchema)) dto: UpdateNotificationTemplatesDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'bodyHtml', label: 'Body Html', type: 'string' },
    { key: 'variables', label: 'Variables', type: 'string' },
    { key: 'senderId', label: 'Sender Id', type: 'string' },
    { key: 'dltTemplateId', label: 'Dlt Template Id', type: 'string' },
    { key: 'provider', label: 'Provider', type: 'number' },
    { key: 'category', label: 'Category', type: 'string' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' },
    { key: 'version', label: 'Version', type: 'number' }
  ];
  private readonly excelSheetName = 'notification_templates';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="notification_templates_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="notification_templates_${Date.now()}.xlsx"`);
    res.send(buf);
  }
