...
import { CreateTripLrsSchema, CreateTripLrsDto } from './dto/create-trip-lrs.dto';
import { UpdateTripLrsSchema, UpdateTripLrsDto } from './dto/update-trip-lrs.dto';
import { FilterTripLrsSchema, FilterTripLrsDto } from './dto/filter-trip-lrs.dto';
...
@ApiTags('operations/trip-lrs')
@ApiBearerAuth()
@Controller('operations/trip-lrs')
export class TripLrsController {
  constructor(private readonly service: TripLrsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateTripLrsSchema)) dto: CreateTripLrsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterTripLrsSchema)) query: FilterTripLrsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateTripLrsSchema)) dto: UpdateTripLrsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'stopSeq', label: 'Stop Seq', type: 'number' },
    { key: 'deliveryAddress', label: 'Delivery Address', type: 'string' },
    { key: 'delLat', label: 'Del Lat', type: 'number' },
    { key: 'delLng', label: 'Del Lng', type: 'number' },
  ...
  private readonly excelSheetName = 'txn_trip_lrs';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="txn_trip_lrs_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="txn_trip_lrs_${Date.now()}.xlsx"`);
    res.send(buf);
  }
