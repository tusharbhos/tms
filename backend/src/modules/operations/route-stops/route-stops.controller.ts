...
import { CreateRouteStopsSchema, CreateRouteStopsDto } from './dto/create-route-stops.dto';
import { UpdateRouteStopsSchema, UpdateRouteStopsDto } from './dto/update-route-stops.dto';
import { FilterRouteStopsSchema, FilterRouteStopsDto } from './dto/filter-route-stops.dto';
...
@ApiTags('operations/route-stops')
@ApiBearerAuth()
@Controller('operations/route-stops')
export class RouteStopsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateRouteStopsSchema)) dto: CreateRouteStopsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterRouteStopsSchema)) query: FilterRouteStopsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateRouteStopsSchema)) dto: UpdateRouteStopsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'stopSequence', label: 'Stop Sequence', type: 'number' },
    { key: 'officeId', label: 'Office Id', type: 'number' },
    { key: 'stopType', label: 'Stop Type', type: 'number' },
    { key: 'estDistanceKm', label: 'Est Distance Km', type: 'number' },
    { key: 'estDurationHrs', label: 'Est Duration Hrs', type: 'number' },
    { key: 'loadingAllowed', label: 'Loading Allowed', type: 'boolean' },
    { key: 'unloadingAllowed', label: 'Unloading Allowed', type: 'boolean' }
  ];
  private readonly excelSheetName = 'txn_route_stops';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="txn_route_stops_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="txn_route_stops_${Date.now()}.xlsx"`);
    res.send(buf);
  }
