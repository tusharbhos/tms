...
import { CreateDriverTasksSchema, CreateDriverTasksDto } from './dto/create-driver-tasks.dto';
import { UpdateDriverTasksSchema, UpdateDriverTasksDto } from './dto/update-driver-tasks.dto';
import { FilterDriverTasksSchema, FilterDriverTasksDto } from './dto/filter-driver-tasks.dto';
...
@ApiTags('operations/driver-tasks')
@ApiBearerAuth()
@Controller('operations/driver-tasks')
export class DriverTasksController {
  constructor(private readonly service: DriverTasksService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateDriverTasksSchema)) dto: CreateDriverTasksDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterDriverTasksSchema)) query: FilterDriverTasksDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateDriverTasksSchema)) dto: UpdateDriverTasksDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'taskType', label: 'Task Type', type: 'date' },
    { key: 'taskTitle', label: 'Task Title', type: 'string' },
    { key: 'taskDetails', label: 'Task Details', type: 'string' },
    { key: 'dueAt', label: 'Due At', type: 'date' },
    { key: 'status', label: 'Status', type: 'string' },
    { key: 'priority', label: 'Priority', type: 'string' },
    { key: 'reminderCount', label: 'Reminder Count', type: 'number' },
    { key: 'lastRemindedAt', label: 'Last Reminded At', type: 'date' },
    { key: 'completedAt', label: 'Completed At', type: 'date' },
    { key: 'completedNote', label: 'Completed Note', type: 'string' }
  ];
  private readonly excelSheetName = 'txn_driver_tasks';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="txn_driver_tasks_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="txn_driver_tasks_${Date.now()}.xlsx"`);
    res.send(buf);
  }
