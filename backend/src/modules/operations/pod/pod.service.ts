...
import { FilterPodDto } from './dto/filter-pod.dto';
import { PodTransitions } from './entities/pod.entity';
import { PrismaService } from '../../../database/prisma.service';

// Pod service — includes finite STATE MACHINE validation.
// Allowed transitions come from PodTransitions (generated from the TMS spec).
@Injectable()
export class PodService {
  ...
  create(tenantId: number, userId: number, dto: CreatePodDto) {
    return this.repo.create(tenantId, userId, dto);
  }

  findAll(tenantId: number, query: FilterPodDto) {
    return this.repo.findAll(tenantId, query);
  }

  async findOne(tenantId: number, id: string) {
    const row = await this.repo.findOne(tenantId, id);
    if (!row) throw new NotFoundException('Pod not found');
    return row;
  }

  update(tenantId: number, userId: number, id: string, dto: UpdatePodDto) {
  ...
  /**
   * Transition the podVerificationStatus to a new state with full validation:
  ...
  async transition(
    ...
    const from: string = row.podVerificationStatus;

    const allowed = PodTransitions[from] ?? [];
    if (!allowed.includes(to)) {
      throw new BadRequestException(
        `Illegal podVerificationStatus transition: ${from} -> ${to}. Allowed: ${allowed.join(', ') || 'none'}`,
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      const updated = await (tx as any).pod.update({
        where: { id: BigInt(id) as any },
        data: { podVerificationStatus: to, updatedBy: userId },
      });
