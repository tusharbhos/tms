...
import { CreateVendorVoucherItemsSchema, CreateVendorVoucherItemsDto } from './dto/create-vendor-voucher-items.dto';
import { UpdateVendorVoucherItemsSchema, UpdateVendorVoucherItemsDto } from './dto/update-vendor-voucher-items.dto';
import { FilterVendorVoucherItemsSchema, FilterVendorVoucherItemsDto } from './dto/filter-vendor-voucher-items.dto';
...
@ApiTags('finance/vendor-voucher-items')
@ApiBearerAuth()
@Controller('finance/vendor-voucher-items')
export class VendorVoucherItemsController {
  constructor(private readonly service: VendorVoucherItemsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateVendorVoucherItemsSchema)) dto: CreateVendorVoucherItemsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterVendorVoucherItemsSchema)) query: FilterVendorVoucherItemsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateVendorVoucherItemsSchema)) dto: UpdateVendorVoucherItemsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    { key: 'voucherId', label: 'Voucher Id', type: 'number' },
    { key: 'lineType', label: 'Line Type', type: 'string' },
    { key: 'description', label: 'Description', type: 'string' },
    { key: 'amount', label: 'Amount', type: 'number' },
    { key: 'referenceId', label: 'Reference Id', type: 'number' },
    { key: 'referenceType', label: 'Reference Type', type: 'string' }
  ];
  private readonly excelSheetName = 'txn_vendor_voucher_items';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="txn_vendor_voucher_items_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="txn_vendor_voucher_items_${Date.now()}.xlsx"`);
    res.send(buf);
  }
