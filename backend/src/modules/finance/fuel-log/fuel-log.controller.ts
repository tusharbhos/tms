...
import { CreateFuelLogSchema, CreateFuelLogDto } from './dto/create-fuel-log.dto';
import { UpdateFuelLogSchema, UpdateFuelLogDto } from './dto/update-fuel-log.dto';
import { FilterFuelLogSchema, FilterFuelLogDto } from './dto/filter-fuel-log.dto';
...
@ApiTags('finance/fuel-log')
@ApiBearerAuth()
@Controller('finance/fuel-log')
export class FuelLogController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateFuelLogSchema)) dto: CreateFuelLogDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterFuelLogSchema)) query: FilterFuelLogDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateFuelLogSchema)) dto: UpdateFuelLogDto,
  ) {
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="fm_fuel_log_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="fm_fuel_log_${Date.now()}.xlsx"`);
    res.send(buf);
  }
