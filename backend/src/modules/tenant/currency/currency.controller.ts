...
import { CreateCurrencySchema, CreateCurrencyDto } from './dto/create-currency.dto';
import { UpdateCurrencySchema, UpdateCurrencyDto } from './dto/update-currency.dto';
import { FilterCurrencySchema, FilterCurrencyDto } from './dto/filter-currency.dto';
...
@ApiTags('tenant/currency')
@ApiBearerAuth()
@Controller('tenant/currency')
export class CurrencyController {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterCurrencySchema)) query: FilterCurrencyDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateCurrencySchema)) dto: UpdateCurrencyDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'symbolPosition', label: 'Symbol Position', type: 'string' },
    { key: 'thousandsSeparator', label: 'Thousands Separator', type: 'string' },
    { key: 'decimalSeparator', label: 'Decimal Separator', type: 'string' },
    { key: 'active', label: 'Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'currency';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="currency_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="currency_${Date.now()}.xlsx"`);
    res.send(buf);
  }
