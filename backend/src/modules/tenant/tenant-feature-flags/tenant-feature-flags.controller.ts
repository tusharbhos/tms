...
import { CreateTenantFeatureFlagsSchema, CreateTenantFeatureFlagsDto } from './dto/create-tenant-feature-flags.dto';
import { UpdateTenantFeatureFlagsSchema, UpdateTenantFeatureFlagsDto } from './dto/update-tenant-feature-flags.dto';
import { FilterTenantFeatureFlagsSchema, FilterTenantFeatureFlagsDto } from './dto/filter-tenant-feature-flags.dto';
...
@ApiTags('tenant/tenant-feature-flags')
@ApiBearerAuth()
@Controller('tenant/tenant-feature-flags')
export class TenantFeatureFlagsController {
  constructor(private readonly service: TenantFeatureFlagsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateTenantFeatureFlagsSchema)) dto: CreateTenantFeatureFlagsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterTenantFeatureFlagsSchema)) query: FilterTenantFeatureFlagsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateTenantFeatureFlagsSchema)) dto: UpdateTenantFeatureFlagsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    { key: 'featureFlagId', label: 'Feature Flag Id', type: 'number' },
    { key: 'featureKey', label: 'Feature Key', type: 'string' },
    { key: 'enabled', label: 'Enabled', type: 'boolean' },
    { key: 'limitValue', label: 'Limit Value', type: 'number' },
    { key: 'overrideValue', label: 'Override Value', type: 'string' },
    { key: 'validFrom', label: 'Valid From', type: 'date' },
    { key: 'validUntil', label: 'Valid Until', type: 'date' },
    { key: 'notes', label: 'Notes', type: 'string' }
  ];
  private readonly excelSheetName = 'tenant_feature_flags';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="tenant_feature_flags_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="tenant_feature_flags_${Date.now()}.xlsx"`);
    res.send(buf);
  }
