...
import { FilterTenantSchema, FilterTenantDto } from './dto/filter-tenant.dto';
...
export class TenantController {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterTenantSchema)) query: FilterTenantDto,
  ) {
  ...
  private readonly excelSheetName = 'tenant';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="tenant_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="tenant_${Date.now()}.xlsx"`);
    res.send(buf);
  }
