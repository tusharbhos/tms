...
import { CreateOrganizationsSchema, CreateOrganizationsDto } from './dto/create-organizations.dto';
import { UpdateOrganizationsSchema, UpdateOrganizationsDto } from './dto/update-organizations.dto';
import { FilterOrganizationsSchema, FilterOrganizationsDto } from './dto/filter-organizations.dto';
...
export class OrganizationsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateOrganizationsSchema)) dto: CreateOrganizationsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterOrganizationsSchema)) query: FilterOrganizationsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateOrganizationsSchema)) dto: UpdateOrganizationsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    { key: 'parentOrganizationId', label: 'Parent Organization Id', type: 'number' },
    { key: 'orgCode', label: 'Org Code', type: 'string' },
    { key: 'orgName', label: 'Org Name', type: 'string' },
    { key: 'orgNameReg', label: 'Org Name Reg', type: 'string' },
    { key: 'orgType', label: 'Org Type', type: 'string' },
    { key: 'description', label: 'Description', type: 'string' },
    { key: 'logoUrl', label: 'Logo Url', type: 'string' },
    { key: 'country', label: 'Country', type: 'string' },
    { key: 'status', label: 'Status', type: 'string' }
  ];
  private readonly excelSheetName = 'organizations';
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="organizations_${Date.now()}.xlsx"`);
    res.send(buf);
  }
