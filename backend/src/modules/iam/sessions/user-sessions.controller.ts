...
import { CreateUserSessionsSchema, CreateUserSessionsDto } from './dto/create-user-sessions.dto';
import { UpdateUserSessionsSchema, UpdateUserSessionsDto } from './dto/update-user-sessions.dto';
import { FilterUserSessionsSchema, FilterUserSessionsDto } from './dto/filter-user-sessions.dto';
...
export class UserSessionsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateUserSessionsSchema)) dto: CreateUserSessionsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterUserSessionsSchema)) query: FilterUserSessionsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateUserSessionsSchema)) dto: UpdateUserSessionsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'sessionTokenHash', label: 'Session Token Hash', type: 'string' },
    ...
    { key: 'locationLat', label: 'Location Lat', type: 'number' },
    { key: 'locationLng', label: 'Location Lng', type: 'number' },
    { key: 'sessionStartedAt', label: 'Session Started At', type: 'date' },
    { key: 'lastActiveAt', label: 'Last Active At', type: 'date' },
    { key: 'sessionEndedAt', label: 'Session Ended At', type: 'date' },
    { key: 'endReason', label: 'End Reason', type: 'date' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'user_sessions';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="user_sessions_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="user_sessions_${Date.now()}.xlsx"`);
    res.send(buf);
  }
