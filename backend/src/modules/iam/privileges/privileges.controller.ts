...
import { CreatePrivilegesSchema, CreatePrivilegesDto } from './dto/create-privileges.dto';
import { UpdatePrivilegesSchema, UpdatePrivilegesDto } from './dto/update-privileges.dto';
import { FilterPrivilegesSchema, FilterPrivilegesDto } from './dto/filter-privileges.dto';
...
@ApiTags('iam/privileges')
@ApiBearerAuth()
@Controller('iam/privileges')
export class PrivilegesController {
  constructor(private readonly service: PrivilegesService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreatePrivilegesSchema)) dto: CreatePrivilegesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterPrivilegesSchema)) query: FilterPrivilegesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdatePrivilegesSchema)) dto: UpdatePrivilegesDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'apiEndpoint', label: 'Api Endpoint', type: 'string' },
    { key: 'parentPrivilegeId', label: 'Parent Privilege Id', type: 'number' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'privileges';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="privileges_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="privileges_${Date.now()}.xlsx"`);
    res.send(buf);
  }
