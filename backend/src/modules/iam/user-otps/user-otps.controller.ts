...
import { CreateUserOtpsSchema, CreateUserOtpsDto } from './dto/create-user-otps.dto';
import { UpdateUserOtpsSchema, UpdateUserOtpsDto } from './dto/update-user-otps.dto';
import { FilterUserOtpsSchema, FilterUserOtpsDto } from './dto/filter-user-otps.dto';
...
@ApiTags('iam/user-otps')
@ApiBearerAuth()
@Controller('iam/user-otps')
export class UserOtpsController {
  constructor(private readonly service: UserOtpsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateUserOtpsSchema)) dto: CreateUserOtpsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterUserOtpsSchema)) query: FilterUserOtpsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateUserOtpsSchema)) dto: UpdateUserOtpsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'otpHash', label: 'Otp Hash', type: 'string' },
    { key: 'expiresAt', label: 'Expires At', type: 'date' },
    { key: 'failedOtpLoginAttempts', label: 'Failed Otp Login Attempts', type: 'number' },
    { key: 'otpLoginBlockedTill', label: 'Otp Login Blocked Till', type: 'date' }
  ];
  private readonly excelSheetName = 'user_otps';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="user_otps_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="user_otps_${Date.now()}.xlsx"`);
    res.send(buf);
  }
