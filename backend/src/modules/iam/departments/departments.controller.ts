...
import { CreateDepartmentsSchema, CreateDepartmentsDto } from './dto/create-departments.dto';
import { UpdateDepartmentsSchema, UpdateDepartmentsDto } from './dto/update-departments.dto';
import { FilterDepartmentsSchema, FilterDepartmentsDto } from './dto/filter-departments.dto';
...
export class DepartmentsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateDepartmentsSchema)) dto: CreateDepartmentsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterDepartmentsSchema)) query: FilterDepartmentsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateDepartmentsSchema)) dto: UpdateDepartmentsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    { key: 'parentDepartmentId', label: 'Parent Department Id', type: 'number' },
    { key: 'departmentCode', label: 'Department Code', type: 'string' },
    { key: 'name', label: 'Name', type: 'string' },
    { key: 'hodUserId', label: 'Hod User Id', type: 'number' },
    { key: 'description', label: 'Description', type: 'string' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'departments';
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="departments_${Date.now()}.xlsx"`);
    res.send(buf);
  }
