...
import { CreateRolePrivilegesSchema, CreateRolePrivilegesDto } from './dto/create-role-privileges.dto';
import { UpdateRolePrivilegesSchema, UpdateRolePrivilegesDto } from './dto/update-role-privileges.dto';
import { FilterRolePrivilegesSchema, FilterRolePrivilegesDto } from './dto/filter-role-privileges.dto';
...
@ApiTags('iam/role-privileges')
@ApiBearerAuth()
@Controller('iam/role-privileges')
export class RolePrivilegesController {
  constructor(private readonly service: RolePrivilegesService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateRolePrivilegesSchema)) dto: CreateRolePrivilegesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterRolePrivilegesSchema)) query: FilterRolePrivilegesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateRolePrivilegesSchema)) dto: UpdateRolePrivilegesDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'privilegeId', label: 'Privilege Id', type: 'number' }
  ];
  private readonly excelSheetName = 'role_privileges';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="role_privileges_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="role_privileges_${Date.now()}.xlsx"`);
    res.send(buf);
  }
