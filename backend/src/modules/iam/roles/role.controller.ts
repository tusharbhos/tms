...
import { CreateRoleSchema, CreateRoleDto } from './dto/create-role.dto';
import { UpdateRoleSchema, UpdateRoleDto } from './dto/update-role.dto';
import { FilterRoleSchema, FilterRoleDto } from './dto/filter-role.dto';
...
export class RoleController {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterRoleSchema)) query: FilterRoleDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'roleLevel', label: 'Role Level', type: 'number' },
    { key: 'isSystemRole', label: 'Is System Role', type: 'boolean' },
    { key: 'officeScope', label: 'Office Scope', type: 'string' },
    { key: 'departmentScope', label: 'Department Scope', type: 'string' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'role';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="role_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="role_${Date.now()}.xlsx"`);
    res.send(buf);
  }
