...
import { CreateClaimsSchema, CreateClaimsDto } from './dto/create-claims.dto';
import { UpdateClaimsSchema, UpdateClaimsDto } from './dto/update-claims.dto';
import { FilterClaimsSchema, FilterClaimsDto } from './dto/filter-claims.dto';
...
@ApiTags('tracking/claims')
@ApiBearerAuth()
@Controller('tracking/claims')
export class ClaimsController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateClaimsSchema)) dto: CreateClaimsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterClaimsSchema)) query: FilterClaimsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateClaimsSchema)) dto: UpdateClaimsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'claimStatus', label: 'Claim Status', type: 'string' },
    { key: 'reviewedBy', label: 'Reviewed By', type: 'number' },
  ...
  private readonly excelSheetName = 'txn_claims';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="txn_claims_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="txn_claims_${Date.now()}.xlsx"`);
    res.send(buf);
  }
