...
import { CreateVendorKycSchema, CreateVendorKycDto } from './dto/create-vendor-kyc.dto';
import { UpdateVendorKycSchema, UpdateVendorKycDto } from './dto/update-vendor-kyc.dto';
import { FilterVendorKycSchema, FilterVendorKycDto } from './dto/filter-vendor-kyc.dto';
...
@ApiTags('masters/vendor-kyc')
@ApiBearerAuth()
@Controller('masters/vendor-kyc')
export class VendorKycController {
  constructor(private readonly service: VendorKycService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateVendorKycSchema)) dto: CreateVendorKycDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterVendorKycSchema)) query: FilterVendorKycDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateVendorKycSchema)) dto: UpdateVendorKycDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'dlNum', label: 'Dl Num', type: 'string' },
    { key: 'dlIssueDate', label: 'Dl Issue Date', type: 'date' },
    { key: 'dlExpiryDate', label: 'Dl Expiry Date', type: 'date' },
    { key: 'dlIssueRto', label: 'Dl Issue Rto', type: 'string' },
    { key: 'dlCardUrl', label: 'Dl Card Url', type: 'string' },
  ...
  private readonly excelSheetName = 'vendor_kyc';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="vendor_kyc_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="vendor_kyc_${Date.now()}.xlsx"`);
    res.send(buf);
  }
