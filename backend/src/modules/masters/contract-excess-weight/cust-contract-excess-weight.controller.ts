...
import { CreateCustContractExcessWeightSchema, CreateCustContractExcessWeightDto } from './dto/create-cust-contract-excess-weight.dto';
import { UpdateCustContractExcessWeightSchema, UpdateCustContractExcessWeightDto } from './dto/update-cust-contract-excess-weight.dto';
import { FilterCustContractExcessWeightSchema, FilterCustContractExcessWeightDto } from './dto/filter-cust-contract-excess-weight.dto';
...
@ApiTags('masters/contract-excess-weight')
@ApiBearerAuth()
@Controller('masters/contract-excess-weight')
export class CustContractExcessWeightController {
  constructor(private readonly service: CustContractExcessWeightService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateCustContractExcessWeightSchema)) dto: CreateCustContractExcessWeightDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterCustContractExcessWeightSchema)) query: FilterCustContractExcessWeightDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateCustContractExcessWeightSchema)) dto: UpdateCustContractExcessWeightDto,
  ) {
  ...
  private readonly excelSheetName = 'cust_contract_excess_weight';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="cust_contract_excess_weight_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="cust_contract_excess_weight_${Date.now()}.xlsx"`);
    res.send(buf);
  }
