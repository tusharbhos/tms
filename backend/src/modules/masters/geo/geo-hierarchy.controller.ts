...
import { CreateGeoHierarchySchema, CreateGeoHierarchyDto } from './dto/create-geo-hierarchy.dto';
import { UpdateGeoHierarchySchema, UpdateGeoHierarchyDto } from './dto/update-geo-hierarchy.dto';
import { FilterGeoHierarchySchema, FilterGeoHierarchyDto } from './dto/filter-geo-hierarchy.dto';
...
@ApiTags('masters/geo')
@ApiBearerAuth()
@Controller('masters/geo')
export class GeoHierarchyController {
  constructor(private readonly service: GeoHierarchyService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateGeoHierarchySchema)) dto: CreateGeoHierarchyDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterGeoHierarchySchema)) query: FilterGeoHierarchyDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateGeoHierarchySchema)) dto: UpdateGeoHierarchyDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'poName', label: 'Po Name', type: 'string' },
    { key: 'pincode', label: 'Pincode', type: 'string' },
    { key: 'poLat', label: 'Po Lat', type: 'number' },
    { key: 'poLong', label: 'Po Long', type: 'number' },
    { key: 'place', label: 'Place', type: 'string' },
    { key: 'placeLat', label: 'Place Lat', type: 'number' },
    { key: 'placeLong', label: 'Place Long', type: 'number' },
    { key: 'isActive', label: 'Is Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'geo_hierarchy';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="geo_hierarchy_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="geo_hierarchy_${Date.now()}.xlsx"`);
    res.send(buf);
  }
