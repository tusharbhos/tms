...
import { CreateDriverRateSchema, CreateDriverRateDto } from './dto/create-driver-rate.dto';
import { UpdateDriverRateSchema, UpdateDriverRateDto } from './dto/update-driver-rate.dto';
import { FilterDriverRateSchema, FilterDriverRateDto } from './dto/filter-driver-rate.dto';
...
@ApiTags('masters/driver-rate')
@ApiBearerAuth()
@Controller('masters/driver-rate')
export class DriverRateController {
  constructor(private readonly service: DriverRateService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateDriverRateSchema)) dto: CreateDriverRateDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterDriverRateSchema)) query: FilterDriverRateDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateDriverRateSchema)) dto: UpdateDriverRateDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'perExtraKmRate', label: 'Per Extra Km Rate', type: 'number' },
    { key: 'nightHaltRate', label: 'Night Halt Rate', type: 'number' },
    { key: 'perTripRate', label: 'Per Trip Rate', type: 'number' },
    { key: 'tripAllowance', label: 'Trip Allowance', type: 'number' },
    { key: 'incentivePerTrip', label: 'Incentive Per Trip', type: 'number' },
    { key: 'monthlySal', label: 'Monthly Sal', type: 'number' },
    { key: 'monthlyIncentive', label: 'Monthly Incentive', type: 'number' },
    { key: 'perTripPenaltyPercent', label: 'Per Trip Penalty Percent', type: 'number' },
    { key: 'perTripPenaltyFixedAmount', label: 'Per Trip Penalty Fixed Amount', type: 'number' },
  ...
  private readonly excelSheetName = 'driver_rate';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="driver_rate_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="driver_rate_${Date.now()}.xlsx"`);
    res.send(buf);
  }
