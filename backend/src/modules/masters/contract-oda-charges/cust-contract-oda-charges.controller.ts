...
import { CreateCustContractOdaChargesSchema, CreateCustContractOdaChargesDto } from './dto/create-cust-contract-oda-charges.dto';
import { UpdateCustContractOdaChargesSchema, UpdateCustContractOdaChargesDto } from './dto/update-cust-contract-oda-charges.dto';
import { FilterCustContractOdaChargesSchema, FilterCustContractOdaChargesDto } from './dto/filter-cust-contract-oda-charges.dto';
...
@ApiTags('masters/contract-oda-charges')
@ApiBearerAuth()
@Controller('masters/contract-oda-charges')
export class CustContractOdaChargesController {
  constructor(private readonly service: CustContractOdaChargesService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateCustContractOdaChargesSchema)) dto: CreateCustContractOdaChargesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterCustContractOdaChargesSchema)) query: FilterCustContractOdaChargesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateCustContractOdaChargesSchema)) dto: UpdateCustContractOdaChargesDto,
  ) {
  ...
  private readonly excelSheetName = 'cust_contract_oda_charges';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="cust_contract_oda_charges_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="cust_contract_oda_charges_${Date.now()}.xlsx"`);
    res.send(buf);
  }
