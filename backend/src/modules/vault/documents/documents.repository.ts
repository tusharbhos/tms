...
// Repository — all DB access for `documents`.
// Tenant isolation (RLS) is enforced: every query is scoped by tenantId.
@Injectable()
export class DocumentsRepository {
  constructor(private readonly prisma: PrismaService) {}

  async create(tenantId: number, userId: number, dto: CreateDocumentsDto) {
    return this.prisma.documents.create({
      data: { ...dto, tenantId, createdBy: userId } as any,
    });
  }

  async findAll(tenantId: number, query: FilterDocumentsDto) {
  ...
  async update(tenantId: number, userId: number, id: string | number, dto: UpdateDocumentsDto) {
