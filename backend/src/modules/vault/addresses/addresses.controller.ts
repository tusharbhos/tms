...
import { CreateAddressesSchema, CreateAddressesDto } from './dto/create-addresses.dto';
import { UpdateAddressesSchema, UpdateAddressesDto } from './dto/update-addresses.dto';
import { FilterAddressesSchema, FilterAddressesDto } from './dto/filter-addresses.dto';
...
@ApiTags('vault/addresses')
@ApiBearerAuth()
@Controller('vault/addresses')
export class AddressesController {
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateAddressesSchema)) dto: CreateAddressesDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterAddressesSchema)) query: FilterAddressesDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateAddressesSchema)) dto: UpdateAddressesDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'addressType', label: 'Address Type', type: 'string' },
    { key: 'addressLine1', label: 'Address Line1', type: 'string' },
    { key: 'addressLine2', label: 'Address Line2', type: 'string' },
    ...
    { key: 'geoHierarchyId', label: 'Geo Hierarchy Id', type: 'number' },
    { key: 'addressReg', label: 'Address Reg', type: 'string' },
    { key: 'proofDocUrl', label: 'Proof Doc Url', type: 'string' },
    { key: 'status', label: 'Status', type: 'string' }
  ];
  private readonly excelSheetName = 'addresses';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="addresses_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="addresses_${Date.now()}.xlsx"`);
    res.send(buf);
  }
