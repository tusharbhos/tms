...
import { CreateTaxRegistrationsSchema, CreateTaxRegistrationsDto } from './dto/create-tax-registrations.dto';
import { UpdateTaxRegistrationsSchema, UpdateTaxRegistrationsDto } from './dto/update-tax-registrations.dto';
import { FilterTaxRegistrationsSchema, FilterTaxRegistrationsDto } from './dto/filter-tax-registrations.dto';
...
@ApiTags('vault/tax-registrations')
@ApiBearerAuth()
@Controller('vault/tax-registrations')
export class TaxRegistrationsController {
  constructor(private readonly service: TaxRegistrationsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateTaxRegistrationsSchema)) dto: CreateTaxRegistrationsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterTaxRegistrationsSchema)) query: FilterTaxRegistrationsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateTaxRegistrationsSchema)) dto: UpdateTaxRegistrationsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'regType', label: 'Reg Type', type: 'string' },
    { key: 'regNumber', label: 'Reg Number', type: 'string' },
    { key: 'regName', label: 'Reg Name', type: 'string' },
    { key: 'state', label: 'State', type: 'string' },
    { key: 'regDate', label: 'Reg Date', type: 'date' },
    { key: 'expiryDate', label: 'Expiry Date', type: 'date' },
    { key: 'docUrl', label: 'Doc Url', type: 'string' },
    { key: 'docTypeId', label: 'Doc Type Id', type: 'number' },
  ...
  private readonly excelSheetName = 'tax_registrations';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="tax_registrations_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="tax_registrations_${Date.now()}.xlsx"`);
    res.send(buf);
  }
