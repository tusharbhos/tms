...
import { CreateApiIntegrationsSchema, CreateApiIntegrationsDto } from './dto/create-api-integrations.dto';
import { UpdateApiIntegrationsSchema, UpdateApiIntegrationsDto } from './dto/update-api-integrations.dto';
import { FilterApiIntegrationsSchema, FilterApiIntegrationsDto } from './dto/filter-api-integrations.dto';
...
@ApiTags('integrations/api-integrations')
@ApiBearerAuth()
@Controller('integrations/api-integrations')
export class ApiIntegrationsController {
  constructor(private readonly service: ApiIntegrationsService,
  ...
  create(
    ...
    @Body(new ZodValidationPipe(CreateApiIntegrationsSchema)) dto: CreateApiIntegrationsDto,
  ) {
  ...
  findAll(
    @CurrentTenant() tenantId: number,
    @Query(new ZodValidationPipe(FilterApiIntegrationsSchema)) query: FilterApiIntegrationsDto,
  ) {
  ...
  update(
    ...
    @CurrentUser('id') userId: number,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateApiIntegrationsSchema)) dto: UpdateApiIntegrationsDto,
  ) {
  ...
  private readonly excelColumns: ColumnDef[] = [
    ...
    { key: 'oauthToken', label: 'Oauth Token', type: 'string' },
    { key: 'rateLimitPerMin', label: 'Rate Limit Per Min', type: 'number' },
    { key: 'retryPolicy', label: 'Retry Policy', type: 'number' },
    { key: 'timeoutSeconds', label: 'Timeout Seconds', type: 'number' },
    { key: 'active', label: 'Active', type: 'boolean' }
  ];
  private readonly excelSheetName = 'api_integrations';
  ...
  async downloadTemplate(@Res() res: any) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="api_integrations_template.xlsx"`);
    res.send(buf);
  }
  ...
  async exportExcel(
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="api_integrations_${Date.now()}.xlsx"`);
    res.send(buf);
  }
